...
import akka.projection.r2dbc.internal.PidSeqNr
...
class R2dbcTimestampOffsetStoreSpec
  ...
  "The R2dbcOffsetStore for TimestampOffset" must {
    ...
    "save TimestampOffset with one entry" in {
      ...
      offsetStore.saveOffset(offset1, Some(PidSeqNr("p1", 3L))).futureValue
      ...
      offsetStore.saveOffset(offset2, Some(PidSeqNr("p1", 4L))).futureValue
    ...
    "save TimestampOffset with several seen entries" in {
      ...
      offsetStore.saveOffset(offset2, Some(PidSeqNr("p3", 6L))).futureValue
      val readOffset2 = offsetStore.readOffset[TimestampOffset]()
    ...
    "save TimestampOffset when same timestamp" in {
      ...
      val offset1 = TimestampOffset(clock.instant(), Map("p1" -> 3L, "p2" -> 1L, "p3" -> 5L))
      offsetStore.saveOffset(offset1, Some(PidSeqNr("p1", 3L))).futureValue
      offsetStore.saveOffset(offset1, Some(PidSeqNr("p2", 1L))).futureValue
      offsetStore.saveOffset(offset1, Some(PidSeqNr("p3", 5L))).futureValue
      val readOffset1 = offsetStore.readOffset[TimestampOffset]()
      readOffset1.futureValue shouldBe Some(offset1)

      // not tick, same timestamp
      val offset2 = TimestampOffset(clock.instant(), Map("p2" -> 2L, "p4" -> 9L))
      offsetStore.saveOffset(offset2, Some(PidSeqNr("p2", 2L))).futureValue
      offsetStore.saveOffset(offset2, Some(PidSeqNr("p4", 9L))).futureValue
      val readOffset2 = offsetStore.readOffset[TimestampOffset]()
      ...
      offsetStore.saveOffset(offset3, Some(PidSeqNr("p1", 4L))).futureValue
    ...
    "not update when earlier seqNr" in {
      ...
      offsetStore.saveOffset(offset2, Some(PidSeqNr("p1", 2L))).futureValue
      val readOffset2 = offsetStore.readOffset[TimestampOffset]()
    ...
    "readOffset from given slices" in {
      ...
      tick()
      val offset1 = TimestampOffset(clock.instant(), Map(p1 -> 3L))
      offsetStore0.saveOffset(offset1, Some(PidSeqNr(p1, 3L))).futureValue
      tick()
      val offset2 = TimestampOffset(clock.instant(), Map(p2 -> 4L))
      offsetStore0.saveOffset(offset2, Some(PidSeqNr(p2, 4L))).futureValue
      tick()
      val offset3 = TimestampOffset(clock.instant(), Map(p3 -> 7L))
      offsetStore0.saveOffset(offset3, Some(PidSeqNr(p3, 7L))).futureValue
      tick()
      val offset4 = TimestampOffset(clock.instant(), Map(p4 -> 5L))
      offsetStore0.saveOffset(offset4, Some(PidSeqNr(p4, 5L))).futureValue
    ...
    "filter duplicates" in {
      ...
      offsetStore.saveOffset(offset3, Some(PidSeqNr("p5", 10L))).futureValue
    ...
    "accept known sequence numbers and reject unknown" in {
      ...
      offsetStore
        .saveOffset(TimestampOffset(startTime.plusMillis(2), Map("p4" -> 2L)), Some(PidSeqNr("p4", 2L)))
        .futureValue
      offsetStore
        .saveOffset(TimestampOffset(startTime.plusMillis(9), Map("p5" -> 8L)), Some(PidSeqNr("p5", 8L)))
        .futureValue
    ...
    "update inflight on error and re-accept element" in {
      ...
      offsetStore
        .saveOffset(TimestampOffset(startTime.plusMillis(1), Map("p1" -> 1L)), Some(PidSeqNr("p1", 1L)))
        .futureValue
      ...
      // and they are removed from inflight once they have been stored
      offsetStore
        .saveOffset(TimestampOffset(startTime.plusMillis(2), Map("p1" -> 3L)), Some(PidSeqNr("p1", 3L)))
        .futureValue
      offsetStore.getInflight() shouldBe empty
    }
    ...
    "evict old records" in {
      ...
      offsetStore.saveOffset(TimestampOffset(startTime, Map(p1 -> 1L)), Some(PidSeqNr(p1, 1L))).futureValue
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(JDuration.ofSeconds(1)), Map(p2 -> 1L)), Some(PidSeqNr(p2, 1L)))
        .futureValue
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(JDuration.ofSeconds(2)), Map(p3 -> 1L)), Some(PidSeqNr(p3, 1L)))
        .futureValue
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(evictInterval), Map(p4 -> 1L)), Some(PidSeqNr(p4, 1L)))
        .futureValue
      offsetStore
        .saveOffset(
      ...
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(timeWindow.minusSeconds(10)), Map(p7 -> 1L)), Some(PidSeqNr(p7, 1L)))
        .futureValue
    ...
    "delete old records" in {
      ...
      offsetStore
        ...
        .futureValue
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(JDuration.ofSeconds(3)), Map(p4 -> 1L)), Some(PidSeqNr(p4, 1L)))
        .futureValue
      ...
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(timeWindow.minusSeconds(2)), Map(p5 -> 1L)), Some(PidSeqNr(p5, 1L)))
        .futureValue
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(timeWindow.minusSeconds(1)), Map(p6 -> 1L)), Some(PidSeqNr(p6, 1L)))
        .futureValue
      // nothing deleted yet
      offsetStore.deleteOldTimestampOffsets().futureValue shouldBe 0
      offsetStore.readOffset().futureValue // this will load from database
      ...
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(timeWindow.plusSeconds(1)), Map(p7 -> 1L)), Some(PidSeqNr(p7, 1L)))
        .futureValue
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(timeWindow.plusSeconds(3)), Map(p8 -> 1L)), Some(PidSeqNr(p8, 1L)))
        .futureValue
    ...
    "periodically delete old records" in {
      ...
      offsetStore.saveOffset(TimestampOffset(startTime, Map(p1 -> 1L)), Some(PidSeqNr(p1, 1L))).futureValue
      offsetStore
        .saveOffset(TimestampOffset(startTime.plus(timeWindow.plusSeconds(1)), Map(p2 -> 1L)), Some(PidSeqNr(p2, 1L)))
        .futureValue
      eventually {
        offsetStore.readOffset().futureValue // this will load from database
