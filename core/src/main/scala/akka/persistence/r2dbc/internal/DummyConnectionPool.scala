...
// FIXME Couldn't get r2dbc-pool working. Connections closed at startup. This is just temporary workaround.
...
class PooledConnection(val delegate: Connection, releaseTo: util.Queue[PooledConnection]) extends Connection {
  ...
  override def beginTransaction(definition: TransactionDefinition): Publisher[Void] =
    delegate.beginTransaction(definition)
  ...
  override def setLockWaitTimeout(timeout: Duration): Publisher[Void] =
    delegate.setLockWaitTimeout(timeout)

  override def setStatementTimeout(timeout: Duration): Publisher[Void] =
    delegate.setStatementTimeout(timeout)
