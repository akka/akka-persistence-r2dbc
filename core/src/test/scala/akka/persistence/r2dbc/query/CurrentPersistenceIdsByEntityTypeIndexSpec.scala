...
class CurrentPersistenceIdsByEntityTypeIndexSpec
    extends ScalaTestWithActorTestKit(
      ConfigFactory
        .parseString("""
        akka.persistence.r2dbc.query.persistence-ids.buffer-size = 20
        akka.persistence.r2dbc.query.persistence-ids.entity-type-index-enabled = on
        """)
  ...
  override protected def beforeAll(): Unit = {
    ...
    probe.receiveMessages(numberOfPids * 3, 30.seconds)
  }

  "Event Sourced currentPersistenceIds with entity-type-index enabled" should {
    ...
    "not leak ids from other entity types" in {
      val entityType = entityTypes(2)
      val result =
        query
          .currentPersistenceIds(entityType = entityType, afterId = None, limit = numberOfPids)
          .runWith(Sink.seq)
          .futureValue
      result.map(PersistenceId.extractEntityType).toSet shouldBe Set(entityType)
    }
